    ...
    async def _delete_reminder(self, ctx, todo_list: str, index: str):
        ...
        if index == "all":
            ...
            if not await all_users_reminders.all():
                await ctx.reply("You don't have anything on your todo list!")
                return

            # Ask if the user really wants to do this
            pred = MessagePredicate.yes_or_no(ctx)
            await ctx.reply("Are you **sure** you want to clear this todo list? (yes/no)")
            try:
            ...
            if pred.result:
                ...
                await ctx.reply("I have left your todo list alone.")
                return
            await all_users_reminders.clear()
            await ctx.reply(f"Todo list `{todo_list}` has been cleared.")
            return
